...
def get_single_dataset(dataset_name, root, domain, train_transform, val_transform, val_ratio=0):
    """
    val_ratio: The ratio of validation data in the training set for random splitting.
               If the value is 0, the validation set will use the same images as the training set.
               This does not work for Digits dataset.
    """
    if dataset_name == "Digits":
        train_dataset = datasets.__dict__[domain[0]](osp.join(root, domain[0]), download=True,
                                                            transform=train_transform)
        val_dataset = test_dataset = datasets.__dict__[domain[0]](osp.join(root, domain[0]), split='test',
